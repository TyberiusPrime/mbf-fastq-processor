...
#[test]
fn test_output_incomplete_marker() {
    use std::fs;
    use std::io::Write;
    ...
    let config1_path = temp_path.join("config1.toml");
    let mut config = fs::File::create(&config1_path).unwrap();
    writeln!(
        config,
        r"[input]
read_1 = 'test1.fq'
...
"
    ...
    let report_path = temp_path.join("output.json");
    assert!(!report_path.exists(), "Report file should not exist yet");
    ...
    let cmd = std::process::Command::new(bin_path)
        ...
        .output()
        .unwrap();
    let incomplete_marker = temp_path.join("output.incompleted");

    let stderr = std::str::from_utf8(&cmd.stderr).unwrap().to_string();
    assert!(stderr.contains("output.json' already exists, "));

    assert!(
        !incomplete_marker.exists(),
        "Incomplete marker Was not created, run did not start."
    );
