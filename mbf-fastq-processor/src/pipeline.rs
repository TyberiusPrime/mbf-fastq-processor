            ...
            if !matches!(transform, Transformation::Progress(_)) {
                ...
                if let Some(new_demultiplex_barcodes) = new_demultiplex_barcodes {
                    ...
                    let local_barcode_to_tag = new_demultiplex_barcodes
                        ...
                        .into_iter()
                        .map(|(k, v)| {
                            let tag = local_name_to_tag.get(&v).expect("tag must exist in local_name_to_tag map");
                            (k, *tag)
                        })
                        .collect();
                ...
                let combiner_thread = thread::Builder::new()
                    ...
                    .spawn(move || {
                        if let Err(e) = parse_interleaved_and_send(
                            input_files.segments.pop().expect("segments must contain at least one element for interleaved input"),
                            &combiner_output_tx,
                            segment_order_len,
                            buffer_size,
                            block_size,
...
impl RunStage2 {
    ...
    pub fn create_stage_threads(self, parsed: &mut Config) -> RunStage3 {
        ...
        for (stage_no, mut stage) in stages.into_iter().enumerate() {
            ...
            let input_rx = channels[stage_no].1.take().expect("channel receiver must exist at stage position");
            let output_tx = channels[stage_no + 1].0.take().expect("channel sender must exist at next stage position");
        ...
        let final_channel = channels[last_channel].1.take().expect("final channel receiver must exist");
        RunStage3 {
            //input_info: self.input_info,
            output_directory: self.output_directory,
            report_html: self.report_html,
...
fn collect_thread_failures(
    ...
    for s in error_collector.lock().expect("mutex lock should not be poisoned").drain(..) {
        stage_errors.push(s);
    }
    for p in threads {
        if let Err(e) = p.join() {
            ...
            thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    for set_of_output_files in &mut output_files.output_segments {
                        if let Err(e) = set_of_output_files.1.lock().expect("mutex lock should not be poisoned").finish() {
                            error_collector
                                .lock()
                                .expect("mutex lock should not be poisoned")
                    ...
                    if let Some(output_html) = output_files.output_reports.html.as_mut() {
                        if let Err(e) = output_html_report(output_html, &json_report.expect("json_report must be Some when html output is enabled")) {
                            error_collector
                                .lock()
                                .expect("mutex lock should not be poisoned")
                    ...
                    // Extract timing data
                    let timings = self.timing_collector.lock().expect("mutex lock should not be poisoned").clone();
                    ...
                    if let Some(timeing_file) = output_files.output_reports.timing.as_mut() {
                        ...
                        if let Err(e) = write!(
                            timeing_file,
                            "{}",
                            serde_json::to_string_pretty(&stats).expect("JSON serialization to string should not fail"),
                        ) {
                            error_collector
                                .lock()
                                .expect("mutex lock should not be poisoned")
...
fn handle_stage(
    ...
    timing_collector.lock().expect("mutex lock should not be poisoned").push(timing);
