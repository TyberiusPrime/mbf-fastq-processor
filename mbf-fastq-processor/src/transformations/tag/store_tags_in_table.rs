    ...
    fn apply(
        ...
        if self.in_labels.is_none() {
            ...
            if self.in_labels.is_none() {
                ...
                for tag in self.in_labels.as_ref().expect("in_labels must be set during initialization") {
                    header.push(tag);
                }
        ...
        while let Some(read) = iter.pseudo_next() {
            ...
            if let Some(writer) = self.output_handles.get_mut(&output_tag).expect("output_handle must exist for tag") {
                let mut record = vec![
                    read.name_without_comment(input_info.comment_insert_char)
                        .to_vec(),
                ];
                for tag in self.in_labels.as_ref().expect("in_labels must be set during initialization") {
                    record.push(match &(block.tags.get(tag).expect("tag must exist in block.tags")[ii]) {
