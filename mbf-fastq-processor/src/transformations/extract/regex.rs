...
impl Step for Regex {
    fn validate_others(
        ...
        let group_hunting_regexp = regex::bytes::Regex::new("[$]\\d+_").expect("hardcoded regex must compile");
    ...
    fn declares_tag_type(&self) -> Option<(String, crate::transformations::TagValueType)> {
        Some((
            ...
            if self.segment_index.expect("segment_index must be set during initialization").is_name() {
    ...
    fn apply(
        ...
        let segment_or_name = self.segment_index.expect("segment_index must be set during initialization");
