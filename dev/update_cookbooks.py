...
# Generate Rust code
out = '''// This file is auto-generated by dev/update_cookbooks.py
...
for i, cb in enumerate(cookbooks, 1):
    ...
    path_escaped = cb["path"].replace('"', '\\"')
    readme_escaped = cb["readme"].replace('\\', '\\\\').replace('"', '\\"')
    toml_escaped = cb["toml"].replace('\\', '\\\\').replace('"', '\\"')

    out += f'''    Cookbook {{
        number: {i},
        name: "{name_escaped}",
        path: "{path_escaped}",
        readme: r#"{readme_escaped}"#,
        toml: r#"{toml_escaped}"#,
    }},
'''
